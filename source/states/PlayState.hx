...
class PlayState extends MusicBeatState
{
	...
	public static var camFollowPos:FlxObject = new FlxObject();
	...
	override public function create()
	{
		...
		*/
		...
		forcedCamPos = new FlxPoint(((dad.x + dad.width / 2) + (boyfriend.x + boyfriend.width / 2)) / 2,
			((dad.y + dad.height / 2) + (boyfriend.y + boyfriend.height / 2)) / 2);
		...
		camFollowPos.setPosition(forcedCamPos.x, forcedCamPos.y);
	...
	function onNoteHit(note:Note, strumline:Strumline)
	{
		...
		if (!strumline.isPlayer && losthealth){
			health -= 0.05;
		} else if(!strumline.isPlayer && !losthealth){
			health = 0.05;
		}
		...
		camDisplaceX = 0;
		if (thisChar.animation.curAnim.name == 'singLEFT')
			camDisplaceX -= camDisplaceExtend;
		if (thisChar.animation.curAnim.name == 'singRIGHT')
			camDisplaceX += camDisplaceExtend;

		camDisplaceY = 0;
		if (thisChar.animation.curAnim.name == 'singDOWN')
			camDisplaceY += camDisplaceExtend;
		if (thisChar.animation.curAnim.name == 'singUP')
			camDisplaceY -= camDisplaceExtend;
	}
	...
	var losthealth:Bool = true;

	var balanca:Bool = false;
	...
	override function update(elapsed:Float)
	{
		...
		CoolUtil.dumbCamPosLerp(camGame, camFollowPos, followLerp);
		...
		switch (SONG.song){
			case 'redballed':
				if (curStep == 549)
					//FlxTween.tween(camHUD, {alpha: 0}, 2, {ease: FlxEase.cubeOut});
					hudBuild.setAlpha(0, 2);
				if (curStep == 670)
					//FlxTween.tween(camHUD, {alpha: 1}, 2, {ease: FlxEase.cubeOut});
					hudBuild.setAlpha(1, 2);
				if (curStep == 803)
					CoolUtil.flash(FlxG.camera, 0.5, 0xFFFFFFFF);
					//balanca = true;
		}
		...
		for(strumline in strumlines.members)
		{
			...
			//strumCameraRoll(strumline.allNotes, true);
		}
	...
	public function followCamera(?char:Character, ?offsetX:Float = 0, ?offsetY:Float = 0)
	{
		...
		var getCenterX = char.getMidpoint().x + 10;
		var getCenterY = char.getMidpoint().y - 10;

		camFollowPos.setPosition(getCenterX + camDisplaceX + (200 * playerMult), getCenterY + camDisplaceY);

		if(char != null)
		{
			//camFollowPos.setPosition(char.getMidpoint().x + (200 * playerMult), char.getMidpoint().y - 20);

			//camFollowPos.x += char.cameraOffset.x * playerMult;
			//camFollowPos.y += char.cameraOffset.y;
		}
	}
